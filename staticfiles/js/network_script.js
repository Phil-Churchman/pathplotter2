...
function moveNode(id, n, coord_x, coord_y, backup) {
  sendAJAXRequest(
    JSON.stringify({ id: id, xpos: coord_x, ypos: coord_y, backup: backup }),
    "/new_pos/"
  );
  ...
  for (let i = 0; i < edited_from_links.length; i++) {
    ...
    edited_from_links[i]["xmid"] =
      (coord_x + edited_from_links[i]["xpos2"]) / 2;
    edited_from_links[i]["ymid"] =
      (coord_y + edited_from_links[i]["ypos2"]) / 2;
  }
  ...
  for (let i = 0; i < edited_to_links.length; i++) {
    ...
    edited_to_links[i]["xmid"] =
      (edited_to_links[i]["xpos1"] + edited_to_links[i]["xpos2"]) / 2;
    edited_to_links[i]["ymid"] =
      (edited_to_links[i]["ypos1"] + edited_to_links[i]["ypos2"]) / 2;
  }
...
function makeDraggable(evt, params) {
  ...
  function mousedownEvent(evt) {
    ...
    if (
      evt.target.classList.contains("node") ||
      evt.target.classList.contains("link_mid") ||
      evt.target.id == "network"
    ) {
      if (evt.ctrlKey) {
        cnrlEvent(evt);
      } else if (evt.altKey) {
    ...
    if (evt.ctrlKey) {
      ...
      if (select_range.length == 1) {
        ...
        function checkSelNodes(node) {
          return (node.show &&
            ((node["xpos"] >= select_range[0][0] &&
              node["xpos"] <= select_range[1][0]) ||
              (node["xpos"] <= select_range[0][0] &&
                node["xpos"] >= select_range[1][0])) &&
            ((node["ypos"] >= select_range[0][1] &&
              node["ypos"] <= select_range[1][1]) ||
              (node["ypos"] <= select_range[0][1] &&
                node["ypos"] >= select_range[1][1]))
          );
        }
        for (i = 0; i < sel_nodes.length; i++) {
          ...
          if (selected_nodes.includes(String(sel_nodes[i]["id"]))) {
            selected_nodes.splice(
              selected_nodes.indexOf(sel_nodes[i]["id"]),
              1
            );
  ...
  function cnrlEvent(evt) {
    ...
    if (classes.contains("node")) {
      ...
      open(
        "../add-node-placed/" +
          String(coord.x.toFixed(0)) +
          "/" +
          String(coord.y.toFixed(0)) +
          "/",
        "_self"
      );
    }
  ...
  function createLink(backup) {
    ...
    var existing_links = document.getElementsByClassName(
      "from_node" + node1.id + " to_node" + node2.id
    );

    if (existing_links.length == 0) {
      if (backup) {
        sendAJAXRequest(
          JSON.stringify({ from_node: ctrl_temp[0], to_node: ctrl_temp[1] }),
          "/add-link-defined/"
        );
      } else {
        sendAJAXRequest(
          JSON.stringify({ from_node: ctrl_temp[0], to_node: ctrl_temp[1] }),
          "/add-link-defined-nobackup/"
        );
      }
      ...
      const new_link = document.createElementNS(
  ...
  function cloneToTop(oldEl) {
    ...
    var dragEls =
      oldEl.ownerDocument.documentElement.querySelectorAll(".draggable");
  ...
  function startDrag(evt) {
    if (evt.target.classList.contains("draggable")) {
      if (selected_nodes.includes(parseInt(evt.target.id))) {
        ...
        for (let i = 0; i < selected_nodes.length; i++) {
          var node = cloneToTop(
            document.getElementById(String(selected_nodes[i]))
          );
    ...
    if (selection_drag_active) {
      ...
      for (let i = 0; i < selected_nodes.length; i++) {
        ...
        if (cx < 10) {
          ...
          cx >
          params["Plot_width"] -
            params["Legend_x_spacing"] -
            params["Legend_box_pad"] * 2 -
            10
        ) {
  ...
  function endDrag(evt) {
    ...
    if (selection_drag_active) {
      ...
      sendAJAXRequest(
        JSON.stringify({
          id: id,
          xmid_draw: coord.x - offset.x,
          ymid_draw: coord.y - offset.y,
        }),
        "/new_mid/"
      );
...
function getNetwork(
    ...
    for (i = 0; i < links.length; i++) {
      ...
      if (l["show"]) {
        var d =
          ...
          String(l["ypos1"]) +
          " q " +
          String(l["xmid"] - l["xpos1"]) +
          "," +
          String(l["ymid"] - l["ypos1"]) +
          " " +
          String(l["xpos2"] - l["xpos1"]) +
          "," +
          String(l["ypos2"] - l["ypos1"]);
        ...
        link.classList.add(
          "link",
          "from_node" + String(links[i]["from_node"]),
          "to_node" + String(links[i]["to_node"])
        );
        link.id = String(l["id"]);
  ...
  if (params["Hide_links"] == false && params["Link_midpoints"]) {
    for (i = 0; i < links.length; i++) {
      if (links[i]["show"]) {
        var cx =
          (links[i]["xmid"] + (links[i]["xpos1"] + links[i]["xpos2"]) / 2) / 2;
        var cy =
          (links[i]["ymid"] + (links[i]["ypos1"] + links[i]["ypos2"]) / 2) / 2;
        var mid = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "circle"
        );
  ...
  if (legend_loop) {
    additional.push(["arrow", "burlywood", "", "In loop"]);
  }
  if (legend_group) {
    additional.push(["arrow", "cornflowerblue", "", "In group"]);
  }
  ...
  if (selected_nodes.length > 2) {
    if (side == "horizontal") {
      selected_nodes = selected_nodes.sort(function (a, b) {
        var pos_a = edited_nodes.filter(function (node) {
          return node.id == a;
        })[0].xpos;
        var pos_b = edited_nodes.filter(function (node) {
          return node.id == b;
        })[0].xpos;
        return pos_a - pos_b;
      });
...
function delSelection() {
  for (let i = 0; i < selected_nodes.length; i++) {
    var node = document.getElementById(String(selected_nodes[i]));
    var from_links = Array.from(
      document.getElementsByClassName("from_node" + String(selected_nodes[i]))
    );
    var to_links = Array.from(
      document.getElementsByClassName("to_node" + String(selected_nodes[i]))
    );
    ...
    sendAJAXRequest(
      JSON.stringify({ ob_type: "node", id: selected_nodes[i], backup: false }),
      "/del-ajax/"
    );
  }
  selected_nodes = [];
