function getGantt(gantt_data, dep_dict) {
  ...
  if (params["Durations"] == "With durations") {
    for (var [key, value] of Object.entries(sequence)) {
      ...
      if (value + durations[key] > x_range) {
        x_range = value + durations[key];
      }
    }
  } else {
  ...
  if (params["Order_by"] == "Time") {
    ...
    sequence = temp_obj;
    var items = Object.keys(sequence).map((key) => {
      return [key, sequence[key]];
    });
    items.sort((first, second) => {
      return first[1] - second[1];
    });
    var gantt_items = items.map((e) => {
      return e[0];
    });
    ...
    for (let i = 0; i < Object.keys(sequence).length; i++) {
      if (Object.keys(sequence)[i].slice(0, 1) == "G") {
        ...
        dict_temp[nodes_rev_dict[Object.keys(sequence)[i]][0]] =
          Object.keys(sequence)[i];
      }
    }
    ...
    gantt_items.sort((first, second) => {
      return dep_dict[second] - dep_dict[first];
    });
  }
  ...
  for (let i = 0; i < gantt_items.length; i++) {
    ...
    if (params["Durations"] == "With durations") {
      if (durations[gantt_items[i]] != 0) {
        var ganttbar = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "rect"
        );
        ...
        ganttbar.setAttribute(
          "width",
          String(x_per_unit * durations[gantt_items[i]])
        );
        ...
        ganttbar.setAttribute(
          "points",
          String(x0 - triangle_width / 2) +
            "," +
            String(y1 - bar_padding) +
            " " +
            String(x0 + triangle_width / 2) +
            "," +
            String(y1 - bar_padding) +
  ...
  if (params["Show_combined"]) {
    ...
    var opacity = 0.4;
    for (let i = 0; i < gantt_items.length; i++) {
      var x0 = g_W + x_per_unit * sequence0[gantt_items[i]];
      if (params["Durations"] == "With durations") {
        var x1 = x0 + x_per_unit * durations[gantt_items[i]];
      } else {
        var x1 = x0 + x_per_unit;
      ...
      if (params["Durations"] == "With durations") {
        if (durations[gantt_items[i]] != 0) {
          ...
          ganttbar.setAttribute("stroke-width", 0);
          ganttbar.setAttribute("stroke", "lightgrey");
          ganttbar.setAttribute("opacity", opacity);
          if (gantt_items[i].slice(0, 1) == "G") {
            ganttbar.setAttribute("fill", "black");
          } else {
            ganttbar.setAttribute("fill", colour_lookup[gantt_items[i]]);
  ...
  function polygon_points(points) {
    ...
    var points1y = parseFloat(
      points1.slice(points1.search(",") + 1, points1.length)
    );
    var points2x = parseFloat(points2.slice(0, points2.search(",")));
    var points2y = parseFloat(
      points2.slice(points2.search(",") + 1, points2.length)
    );
    var points3x = parseFloat(points3.slice(0, points3.search(",")));
    var points3y = parseFloat(
      points3.slice(points3.search(",") + 1, points3.length)
    );
  ...
  if (params["Show_out_seq"]) {
    for (let i = 0; i < links_out_order.length; i++) {
      ...
      var d =
        ...
        "," +
        String(ypos2);
      var outseq = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "path"
      );
  ...
  if (params["X_axis"] != "Number" && params["Durations"] == "With durations") {
    for (i = 0; i < Math.ceil(x_range) + 1; i += tick_int) {
      ...
      if (params["X_axis"] == "Month") {
        ...
        var month =
          months[(months.indexOf(params["Start_month_if_Month"]) + i) % 12];
...
function makeSelectable(evt, node_id_dict, group_id_dict) {
  ...
  svg.addEventListener("mousedown", (evt) => {
    selectEvent(evt, node_id_dict, group_id_dict);
  });
