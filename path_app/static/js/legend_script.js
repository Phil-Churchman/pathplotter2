function legend(colour_ref, params, svg, additional) {
  ...
  var y_limit =
    Math.ceil(num_items / per_row) * y_spacing + y_offset + box_pad * 2;
  ...
  function draw_patch(parent, x, y, patch_size, stroke, width, fill, id) {
    ...
    var patch = document.createElementNS(
  ...
  function draw_rect(parent, x, y, patch_size, stroke, width, fill, id) {
    var rect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
    // patch.setAttribute("r", String(patch_size));
    rect.setAttribute("stroke-width", width);
    rect.setAttribute("stroke", stroke);
    rect.setAttribute("y", String(y - patch_size));
    rect.setAttribute("x", String(x));
    rect.setAttribute("height", String(patch_size * 2));
    rect.setAttribute("width", String(patch_size * 3));
  ...
  function draw_label(parent, x, y, patch_size, x_spacing, id, ptextfull) {
    ...
    if (ptextlines.length == 1) {
      var patchtext = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "text"
      );
      ...
      patchtext.setAttribute("x", String(x + patch_size * 4));
      patchtext.setAttribute(
        "y",
        String(y + patch_size * 1 - 2 - patch_size * 1.2)
      );
      patchtext.setAttribute("style", "text-align:left; font-size:10px");
      patchtext.setAttribute("pointer-events", "none");
      patchtext.classList.add("patchtext");
  ...
  var counter = 0;
  if (Object.keys(colour_ref).length > 0) {
    if (params.Apply_groups) {
      var x =
        params["Plot_width"] - (per_row - (0 % per_row)) * x_spacing - box_pad;
    ...
    for (let i = 0; i < Object.keys(colour_ref).length; i++) {
      ...
      var x =
        params["Plot_width"] -
        (per_row - ((i + group) % per_row)) * x_spacing -
        box_pad;
      var y =
        (Math.floor((i + group) / per_row) + 0.5) * y_spacing +
        y_offset +
        box_pad;
      ...
      draw_patch(
        ...
        "none",
        0,
        colour_ref[Object.keys(colour_ref)[i]],
        "patch" + String(i)
      );
  ...
  for (let i = 0; i < additional.length; i++) {
    ...
    var x =
      params["Plot_width"] -
      (per_row - ((i + group + counter) % per_row)) * x_spacing -
      box_pad;
    var y =
      (Math.floor((i + group + counter) / per_row) + 0.5) * y_spacing +
      y_offset +
      box_pad;
